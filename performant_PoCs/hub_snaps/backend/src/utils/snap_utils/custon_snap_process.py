...
from .helpers import (_send_snap, _track_id, _label_idx_name, _status_is_lost,
                      _roi_center_area_norm, _LOSTMID_STATE, _status_is_tracked)
...
def custom_snap_process(
    ...
    if cond_gate.is_key_enabled("timed") and cond_gate.hit("timed"):
        extras = build_extras(model, det_data, class_names) if det_data is not None else {"model": model}
    ...
    if det_data is not None and num == 0:
        if cond_gate.is_key_enabled("no_detections") and cond_gate.hit("no_detections"):
            if _send_snap("No Detections", producer, frame, ["no_detections"], {"model": model, "reason": "no_detections"}):
    ...
    if det_data is not None and cond_gate.is_key_enabled("low_conf"):
        ...
        if thr is not None:
            ...
            if has_low and cond_gate.hit("low_conf"):
                min_conf = min(float(getattr(d, "confidence", 1.0)) for d in dets) if dets else 1.0
                extras = {
                    "model": model,
                ...
                if _send_snap("Low Confidence", producer, frame, ["low_confidence"], extras, det_data):
                    print(f"[LowConf] Snap sent (min={min_conf:.3f} < thr={float(thr):.3f})")
                ...
                if rc is not None:
                    ...
                    if (margin <= cx <= 1.0 - margin) and (margin <= cy <= 1.0 - margin):
                        if cond_gate.hit("lost_mid"):
                            ...
                            if _send_snap("Lost in middle", producer, frame, ["lost_in_middle"], extras, det_data):
                                fired = True
