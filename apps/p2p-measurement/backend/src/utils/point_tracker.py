...
class PointTracker(dai.node.HostNode):    
    ...
    def build(self, rgb_stream: dai.Node.Output, depth_stream: dai.Node.Output) -> "PointTracker":
    ...
    def add_point(self, x: float, y: float, frame_width: int, frame_height: int):
        ...
        try:
            ...
            if (x < 0 or y < 0 or w <= 0 or h <= 0 or 
                x + w > self.current_frame.shape[1] or y + h > self.current_frame.shape[0]):
                raise ValueError("Bbox out of bounds or invalid dimensions")
                
            if self.current_frame.dtype != np.uint8:
                frame_for_tracker = (self.current_frame * 255).astype(np.uint8) if self.current_frame.dtype == np.float32 else self.current_frame.astype(np.uint8)
            else:
            ...
            if success:
                ...
                x, y, w, h = bbox
                roi = self.current_frame[y:y+h, x:x+w] if y+h <= self.current_frame.shape[0] and x+w <= self.current_frame.shape[1] else None
    ...
    def _calculate_bbox_radius(self, point: Tuple[int, int]) -> int:
        ...
        while bbox_radius < self.max_bbox_radius:
            ...
            if roi_y + roi_h <= self.current_frame.shape[0] and roi_x + roi_w <= self.current_frame.shape[1]:
        ...
        for i, point_data in enumerate(self.points):
            ...
            if self.mode['tracking'] == 2 or (self.mode['tracking'] == 1 and i == 1):
                ...
                if success and not self._is_bbox_out_of_frame(new_bbox):
                    if (self._debounce(old_bbox, new_bbox) and 
                        self._calculate_global_motion() < self.motion_threshold):
                        new_bbox = old_bbox  # Keep old bbox
                        
                    point_data['bbox'] = new_bbox
                    
                    x, y, w, h = [int(v) for v in new_bbox]
                    if (y + h <= self.current_frame.shape[0] and 
                        x + w <= self.current_frame.shape[1]):
    ...
    def _is_bbox_out_of_frame(self, bbox: Tuple) -> bool:
        ...
        return (x < 0 or y < 0 or 
                x + w > self.current_frame.shape[1] or 
    ...
    def _calculate_global_motion(self) -> float:
        ...
        curr_gray = cv2.cvtColor(self.current_frame, cv2.COLOR_BGR2GRAY) if len(self.current_frame.shape) == 3 else self.current_frame
    ...
    def calculate_distance_3d(self, p1: Point, p2: Point, depth_frame: np.ndarray) -> Tuple[float, bool]:
        ...
        depth1 = depth_frame[y1, x1] if y1 < depth_frame.shape[0] and x1 < depth_frame.shape[1] else 0
        depth2 = depth_frame[y2, x2] if y2 < depth_frame.shape[0] and x2 < depth_frame.shape[1] else 0
    ...
    def create_annotations(self, rgb_msg, depth_frame: np.ndarray) -> dai.ImgAnnotations:
        ...
        if len(self.points) == 2:
            ...
            norm_p1 = (p1_coords[0] / self.frame_width, p1_coords[1] / self.frame_height)
            norm_p2 = (p2_coords[0] / self.frame_width, p2_coords[1] / self.frame_height)
