...
class VideoSourceManager:
    ...
    def __init__(
        ...
        self.video_src_out: dai.Node.Output = None
        self.input_node: dai.Node.Output = None
        self.video_enc: dai.node.VideoEncoder = None
        self.frame_cache: FrameCacheNode = None
    ...
    def _build(self):
        ...
        self.frame_cache = self._pipeline.create(FrameCacheNode).build(
            self.video_src_out
        )
    ...
    def _create_replay_source(self):
        ...
        self.video_src_out.link(manip.inputImage)
        self.input_node = manip.out
    ...
    def _create_encoder(self):
        ...
        self.video_enc = self._pipeline.create(dai.node.VideoEncoder)
        self.video_enc.setDefaultProfilePreset(
            fps=self._runtime.fps_limit,
            profile=dai.VideoEncoderProperties.Profile.H264_MAIN,
        )
        self.video_src_out.link(self.video_enc.input)

    def _add_video_topic(self):
        """Register video stream with visualizer."""
        self._config.visualizer.addTopic("Video", self.video_enc.out)
