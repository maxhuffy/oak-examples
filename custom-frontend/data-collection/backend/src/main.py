...
from utils.snap_utils import custom_snap_process
...
with dai.Pipeline(device) as pipeline:
    ...
    snaps_producer = pipeline.create(SnapsProducer).build(
        ...
        running=False,
        process_fn=partial(
            custom_snap_process, class_names=CLASS_NAMES, model=args.model
        ),
    )
    ...
    def snap_collection_service(payload):
        ...
        if start_collection:
            ...
            snaps_producer.setRunning(True)
        else:
            snaps_producer.setRunning(False)
        print("Snap collection state:", snaps_producer._running)
